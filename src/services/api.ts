...
// GET requests
export async function cekUser(userId: string): Promise<ApiResponse<{ exists: boolean } & User>> {
  const url = `${APPS_SCRIPT_URL}?action=cekUser&getUser=${encodeURIComponent(userId)}`;
...
export async function fetchDashboard(userId: string, bulanFilter?: string): Promise<ApiResponse<DashboardData>> {
...
// POST requests
export async function registerUser(userId: string, password: string): Promise<ApiResponse> {
...
export async function loginUser(userId: string, password: string): Promise<ApiResponse<User>> {
  const uuid = getDeviceUUID();
  const formData = new FormData();
...
export async function submitPresensi(presensiData: {
  ...
  if (presensiData.latitude) formData.append('latitude', presensiData.latitude.toString());
  if (presensiData.longitude) formData.append('longitude', presensiData.longitude.toString());
  if (presensiData.photoFileName) formData.append('photoFileName', presensiData.photoFileName);
  if (presensiData.fingerprint) formData.append('fingerprint', presensiData.fingerprint);
